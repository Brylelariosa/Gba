        ...
        if (inv[hotbar_sel] > 0) {
            ...
            for (int k = 0; k < 6; k++) {
                ...
                if ((unsigned)tx < (unsigned)WORLD_W &&
                    (unsigned)ty < (unsigned)WORLD_H &&
                    world[ty][tx] == TILE_AIR) {
                    // Don't place inside player - use full player height including head
                    int bpx = tx * TILE_SIZE, bpy = ty * TILE_SIZE;
                    // Player head is 7 pixels above py, so full bounds are (px, py-7) to (px+PLAYER_W, py+PLAYER_H)
                    int px_left = px;
                    int px_right = px + PLAYER_W;
                    int py_top = py - 7;  // head starts here
                    int py_bottom = py + PLAYER_H;
                    
                    int block_left = bpx;
                    int block_right = bpx + TILE_SIZE;
                    int block_top = bpy;
                    int block_bottom = bpy + TILE_SIZE;
                    
                    int overlap_x = (block_left < px_right) && (block_right > px_left);
                    int overlap_y = (block_top < py_bottom) && (block_bottom > py_top);
...
int main(void) {
    ...
    while (1) {
        // FIX #1: Render FIRST, THEN wait for vblank
        // This prevents tearing by ensuring we draw during vblank
        handle_input();
        update_physics();
        ...
        // Wait for vblank AFTER rendering is complete
        VBlankIntrWait();
    }
