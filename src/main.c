...
// Double buffering - use page flipping
#define VRAM_PAGE0 ((u16*)VRAM)
#define VRAM_PAGE1 ((u16*)(VRAM + 0xA000))  // 240*160*2 = 76800 bytes per page
static u16* active_vram = VRAM_PAGE0;
static u16* back_buffer = VRAM_PAGE1;
...
// ─────────────────────────────────────────────────────────────
//  FAST PIXEL / FILL ROUTINES (using back buffer)
// ─────────────────────────────────────────────────────────────
...
static void draw_tile(int sx, int sy, u16 color) {
    ...
    for (int dy = 0; dy < TILE_SIZE; dy++) {
        ...
        u16 *row = back_buffer + y * SCREEN_W + sx;
...
static void handle_input(void) {
    ...
    if (held & KEY_B) {
        ...
        if (mine_timer >= MINE_DELAY) {
            ...
            // Better priority order: front, front-up, front-down, above, below
            int order[8][2] = {
                {dx,  0},  // front
                {dx, -1},  // front-up
                {dx,  1},  // front-down
    ...
    if (pressed & KEY_R) {
        if (inv[hotbar_sel] > 0) {
            ...
            // Better placement order: front, front-up, front-down, above, below, behind
            int cands[8][2] = {
                {dx,  0},  // front
                {dx, -1},  // front-up
                {dx,  1},  // front-down
            ...
            for (int k = 0; k < 8; k++) {
                ...
                // Check if placement position intersects player
...
int main(void) {
    ...
    // Mode 3 with double buffering
    REG_DISPCNT = MODE_3 | BG2_ON | (1 << 4);  // Enable display frame select
    ...
    while (1) {
        ...
        back_buffer = (back_buffer == VRAM_PAGE0) ? VRAM_PAGE1 : VRAM_PAGE0;
        
        handle_input();
        update_physics();
        
        // Clear back buffer with black
        memset(back_buffer, 0, SCREEN_W * SCREEN_H * 2);
        ...
        if (back_buffer == VRAM_PAGE0)
            REG_DISPCNT = MODE_3 | BG2_ON | (0 << 4);
        else
            REG_DISPCNT = MODE_3 | BG2_ON | (1 << 4);
